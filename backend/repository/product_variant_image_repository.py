...
class ProductVariantImageRepository:
    """
    Repository for ProductVariantImage model.
    Provides async CRUD operations for product variant images and variant-specific queries.
    """
    ...
    async def get_by_id(self, image_id):
        """Retrieve a product variant image by its ID."""
        result = await self.db.execute(
            select(ProductVariantImage).filter(ProductVariantImage.image_id == image_id)
        )
        return result.scalars().first()

    async def get_by_variant_id(self, variant_id):
    ...
    async def save(self, image):
        """Save a new product variant image to the database."""
        self.db.add(image)
    ...
    async def update(self, image_id, **kwargs):
        """Update an existing product variant image by ID with provided fields."""
    ...
    async def delete(self, image_id):
        """Delete a product variant image by its ID."""
        image = await self.get_by_id(image_id)
        if image:
